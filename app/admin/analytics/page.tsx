...
export default async function AdminAnalyticsPage() {
  ...
  const departmentRows = departments as any[];
  ...
  const byDate = Object.values(metricRows.reduce<Record<string, { date: string; activeUsers: number; completions: number }>>((acc, m) => {
  ...
  const byDept = departmentRows.map((dept) => ({ name: dept.name, completions: metricRows.filter((m) => m.departmentId === dept.id).reduce((s: number, m: any) => s + m.moduleCompletions, 0) }));
  const pipeline = Object.values(useCaseRows.reduce<Record<string, { name: string; value: number }>>((acc, item) => {
  ...
  return (
    <PageShell title="Admin Analytics" description="Adoption metrics for active users, completions, prompt practice, tutor usage, submitted use cases, adopted use cases, hours saved, and satisfaction.">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Satisfaction score" value={(totals.satisfactionScore / metricRows.length).toFixed(1)} note="Average survey rating" />
      </div>
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Responsible AI Completion</CardTitle></CardHeader><CardContent><DepartmentBar data={departmentRows.map((d) => ({ name: d.name, completions: Math.round(metricRows.filter((m) => m.departmentId === d.id).at(-1)?.responsibleAiCompletionRate ?? 0) }))} /></CardContent></Card>
      </div>
    </PageShell>
  );
}
